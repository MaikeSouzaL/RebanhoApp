...
function SheetContent({
  ...
  const sideClasses: Record<Side, string> = {
    ...
    top: 'inset-x-0 top-0 max-h-[90dvh] rounded-b-3xl border-b data-[state=open]:slide-in-from-top data-[state=closed]:slide-out-to-top',
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        className={cn(
          'fixed z-50 flex flex-col border-border bg-card shadow-xl data-[state=open]:animate-in data-[state=closed]:animate-out',
          sideClasses[side],
          className,
        )}
        {...props}
      >
        ...
        {showClose && (
          <DialogPrimitive.Close className="absolute right-4 top-4 rounded-lg bg-card/80 p-1 opacity-60 backdrop-blur transition hover:opacity-100 focus:outline-none">
            <X className="size-5" />
